...
3. The main window is a slim desktop browser: back / forward / reload / home,
   address pill, thin load-progress bar, offline/error overlay, pop-ups opened
...
| File | Role |
|---|---|
| `src/main/java/cm/odigital/arenaai/Launcher.java` | `main()` entry — launches `ArenaApplication` (also the `jpackage` entry) |
...
- `target="_blank"` links and `window.open(...)` pop-ups automatically open
  in the system browser to keep the single-window desktop UX.
